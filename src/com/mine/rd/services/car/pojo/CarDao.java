...
public class CarDao extends BaseDao {
	...
	public Map<String, Object> insureanceList(int pn, int ps, Object searchContent){
		...
		if(searchContent != null && !"".equals(searchContent)){
			sql = sql + " and (A.PLATE_NUM like '%"+searchContent+"%' or A.BUS_IS like '%"+searchContent+"%'  or DATE_FORMAT(A.BUS_DATE,\"%Y-%m-%d\") like '%"+searchContent+"%' or A.FORCE_IS like '%"+searchContent+"%'  or DATE_FORMAT(A.FORCE_DATE,\"%Y-%m-%d\") like '%"+searchContent+"%'  ) ";
		}
	...
	public Map<String, Object> carListForYearCheck(int pn, int ps, Object searchContent){
		...
		if(searchContent != null && !"".equals(searchContent)){
			sql = sql + " and (A.PLATE_NUM like '%"+searchContent+"%' or A.ENGINE_NUMBER like '%"+searchContent+"%' or A.DRIVER_YEAR like '%"+searchContent+"%' or A.DRIVER_MONTH like '%"+searchContent+"%' or A.DRIVER_DATE like '%"+searchContent+"%' ) ";
		}
		sql = sql + " order by a.sysdate desc ";
		Page<Record> page = Db.paginate(pn, ps, "select A.*  ", sql );
		List<Record> eps = page.getList();
	...
	public Map<String,Object> getContractBack(String id){
		Record record = Db.findFirst("select * from C_WOBO_CONTRACT_BACK where id = ? ",id);
		return record != null ? record.getColumns() : null;
	}
	...
	public boolean saveContractBack(Map<String,Object> map){
		Db.update("delete from C_WOBO_CONTRACT_BACK where id = ? ",map.get("ID"));
		Record record = new Record();
		record.set("ID", map.get("ID"));
		...
		record.set("USER_NAME", map.get("USER_NAME"));
		record.set("sysdate", getSysdate());
		return Db.save("C_WOBO_CONTRACT_BACK",record);
	}
	...
	public boolean updateCarPart(List<Map<String,Object>> list){
		...
		for(int i = 0 ; i < list.size() ; i++){
			...
			flag = Db.update(sb.toString(),list.get(i).get("DRIVING_LICENSE"),list.get(i).get("MAINTENANCE_SIGN"),list.get(i).get("CAR_KEY"),list.get(i).get("CAR_KEY_NUM"),list.get(i).get("INSURANCE"),list.get(i).get("YEAR_CHECK_FLAG"),list.get(i).get("PLATE_NUM")) >= 0;
			if(!flag){
				break;
			}
		}
