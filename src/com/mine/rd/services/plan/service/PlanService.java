		...
		Db.tx(new IAtom() {
	        ...
	        public boolean run() throws SQLException {
        		try {
	            	...
	            	else if("initPt".equals(getLastMethodName(7))){
	            		initPt();
	            	}
...
//	}
	...
	private void saveHandleSelf(){
		...
		String ifsave = controller.getMyParam("ifsave").toString();
		if("1".equals(ifsave)){
			List<Map<String, Object>> handleSelfs = controller.getMyParamList("initHandleSelfs");
			boolean flag = false;
			boolean flag_handle = false;
			int count = 1;
			dao.deleteHandleSelf(tpId);
			dao.deleteHandleSelfList(tpId);
			for(Map<String, Object> handleSelf : handleSelfs){
				...
				map.put("ID",count+"");
				@SuppressWarnings("unchecked")
				List<Map<String, Object>> HANDLE_LIST = (List<Map<String, Object>>) handleSelf.get("HANDLE_LIST");
